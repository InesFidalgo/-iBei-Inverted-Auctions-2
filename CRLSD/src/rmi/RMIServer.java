...
public class RMIServer extends UnicastRemoteObject implements RMI  {
	...
	new Thread(){
	        public void run(){
	            while (true){
	            	//System.out.println("verifica termino");
	                VerificaTermino();
	            }
	        }
	    }.start(); 
	...
	public String[] RegisterAdmin(String username, String password){
        ...
        Users utilizador = new Users(username, password, true, true);
		System.out.println("ola");
		
		String[] fim =  {"true", "true"}; 
		b.users.add(utilizador);
		b.admin.add(utilizador);
		System.out.println("vai actualizar na lsita admin");
	...
	public void leFichLeilao() throws IOException, ClassNotFoundException {
        ...
        for(int i = 0;i<b.leiloes.size();i++){
        	System.out.println("leilao: "+b.leiloes.get(i).titulo);
        	System.out.println("id do leilao: " +b.leiloes.get(i).id);
        }
	...
	public String[] EditarLeilao(int id, int ano, int mes, int dia, int hora, int minuto, String title, String descricao, int amount ) throws RemoteException
	{
		...
		{
			System.out.println("vai procurar o leilao a editar\n" +obj.id +"o que lá está\n" + "o que procurei: "+id);
			if (obj.getId()==(id))
			{
	...
	{
		System.out.println("entrou no procura por user");
		int count = 0;
		String qualquercoisa = "";
		...
		for(int i=0;i<leiloes.size();i++)
		{
			...
			if (leiloes.get(i).getNome().equals(user))
			{
				...
				System.out.println("vai sair do for");
			}

		}
		qualquercoisa = count + qualquercoisa;
		System.out.println("deu return bem do search no rmi");
			...
			if (obj.getNome().equals(username))
			{
				...
				if (obj.getPassword().equals(password))
				{
					...
					if(obj.pendentes.size()>0){
						...
						System.out.println("MENSAGENS PENDENTES"+pendentes.size());
			 ...
			 int ano, int mes, int dia, int hora, int minuto, int amount) throws RemoteException
	{
		...
		System.out.println("create leilao no RMI");
	...
	public String[] ListarUsersOnline() throws RemoteException {
		...
		qualquercoisa = count + qualquercoisa;
		System.out.println("users online"+qualquercoisa);
		String[] fim = qualquercoisa.split("/");
		return fim;
	}
	@Override
	public String[] ListarLicitacoesUser(String user) throws RemoteException{
		System.out.println("ENTROU NO MYLICI" + user);
		int count2 = 0;
		String qualquercoisa2="";
		for(int i=0;i<leiloes.size();i++){
				for(int j=0;j<leiloes.get(i).getLicitacoes().size(); j++){
					System.out.println("falso"+leiloes.get(i).getLicitacoes().get(j).getUser());
					System.out.println("user dado"+ user +"|");
	        ...
	        for(int i=0;i<b.leiloes.size();i++){
	            ...
	            if((comparaTo(ano,mes,dia,hora,minuto)== -1)||((comparaTo(ano,mes,dia,hora,minuto)== 0))){
	                ...
	                if(b.leiloes.get(i).getLicitacoes().size()>0){
	                                    ...
	                                    System.out.println("mano que ganhou"+ganhou);
	                                    for(int j=0;j<b.users.size();j++){
	                                            if(b.users.get(j).nome.equals(ganhou)){
	...
	@Override
	public String GetVelhos(String user) throws RemoteException{
		System.out.println("ENTROU NO RMI NO GETVELHOS");
		int licita = 0;
		String ganhos = "Leilões Licitados que já acabaram : ";
		String comandoresposta[] = null;
		System.out.println("size acabados"+b.acabados.size());
		for(int i=0;i<b.acabados.size();i++){
			for(int j=0;j<b.acabados.get(i).getLicitacoes().size();j++){
				System.out.println("o que esta lá:  "+b.acabados.get(i).getLicitacoes().get(j).user+" | "+user);
				if(b.acabados.get(i).getLicitacoes().get(j).user.equals(user)){
					
					 ganhos += " " +b.acabados.get(i).getId()+", ";
					licita++;
				}
			}
		...
		String ganhados = "Leilões Ganhos: ";
		
		for(int i =0;i< b.users.size();i++){
			System.out.println("size leiloes ganhos"+b.users.get(i).leiloesganhos.size());
			System.out.println("o que está nos users "+b.users.get(i).nome+" | "+user);
			if(b.users.get(i).nome.equals(user)){
				
			for(int j=0;j< b.users.get(i).leiloesganhos.size();j++){
				ganhados += " "+ b.users.get(i).leiloesganhos.get(j).id+", "; 
				}
			}
		}
		ganhos = ganhos + " "+ ganhados;
	...
	public String[] BanUser(String username) {
		...
		for(int j=0; j<b.leiloes.size();j++){
			System.out.println("esta a procura");
			System.out.println("o que esta lá: "+b.leiloes.get(j).getNome());
			System.out.println("o que eu procurei"+username);
			if((b.leiloes.get(j).getNome().equals(username))&&(b.leiloes.get(j).getNome()!=null)){
		...
		try {
			System.out.println("ACTUALIZOU FICHEIROS NO BANIR");
			actualizaFicheiros();
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		String[] fim = {hello};
		System.out.println("agagou user toto");
		return fim;
	...
	*/
	...
	@Override
	public String Dono(int id) throws RemoteException{
		
		for(int i =0;i< leiloes.size();i++){
			if(leiloes.get(i).id==id){
				return leiloes.get(i).getNome();
			}
		}
		return null;
	...
	@Override
	public String Ultimo(int id) throws RemoteException{
		float menor = leiloes.get(leiloes.size()-1).amount;
		String mano = leiloes.get(leiloes.size()-1).user.nome;
		for(int i=0;i< leiloes.size();i++){
			if(leiloes.get(i).amount<menor){
				menor = leiloes.get(i).amount;
				mano = leiloes.get(i).user.nome;
			}
		}
		return mano;
	...
	@Override 
	public ArrayList<String> licitadores (int id) throws RemoteException{
		System.out.println(id);
		System.out.println("ENTROU NO RMI");
		...
		for(int i=0;i<leiloes.size();i++){
			if(leiloes.get(i).id==id){
				...
				for(int j=0;j<leiloes.get(i).getLicitacoes().size();j++){
					a.add(leiloes.get(i).getLicitacoes().get(j).getUser());
				}
				return a;
			}
		}
	...
	@Override
	public String[] MakeLicitacao(int id, String user, float amount) throws RemoteException {
		System.out.println("vai licitar no rmi");
		...
		for (Leilao obj2 : b.leiloes)
		{
			if (obj2.getId() == id)
			{
				System.out.println("encontrou um leilao com id igual e vai licitar");
				if(amount>0){
				if(((amount<=obj2.amount)))
				{
					System.out.println("licitado:"+amount +"|" + obj2.amount);
					Licitacao nova = new Licitacao(user, id, amount);
				...
				if(obj2.licitacoes.size()>1){
					...
					if(amount<=obj2.licitacoes.get(obj2.licitacoes.size()-1).preco){
						...
						for(int i = 0;i<obj2.licitacoes.size();i++){
							for(int k = 0;k<b.users.size();k++){
								System.out.println("user rmi"+b.users.get(k).getNome()+" | "+"user licitacao"+obj2.licitacoes.get(i).user);
								if(b.users.get(k).getNome().equals(obj2.licitacoes.get(i).user)){
									System.out.println("SIZE PENDENTES"+b.users.get(k).pendentes.size());
									if(b.users.get(k).pendentes.size()>0){
    ...
    public void sair (String username){
        ...
        for(int i=0;i<online.size();i++){
            if(online.get(i).getNome().equals(username)){
            	System.out.println("vai remover"+username);
                online.remove(i);
                System.out.println(online.size());
                for(int j=0;j<online.size();j++){
                	System.out.println("esta online:"+online.get(j));
                }
            }
        } 
    }
	...
	public String[] WriteMensagem(int id, String mensagem, String user) throws RemoteException {
		...
		for (Leilao obj : b.leiloes)
		{
			if (obj.getId() == id)
			{
				Mensagem nova = new Mensagem(user, id, mensagem);
				obj.getMensagens().add(nova);
				...
				for (Users obj2 : b.users)
				{
					if (obj.getUser().getNome().equals(obj2.getNome()))
					{
						...
						for(int i=0;i<b.online.size();i++){
							System.out.println("esta online"+b.online.get(i).nome);
						}
						
						if(cnt==0){
							
							System.out.println("ENTROU NO ELSE ISLOG2"); //user nao está online e fica pendente
...
//FACEBOOK!!!!!!!!!!!!
@Override
public int LoginFacebook(String IDcode) throws RemoteException
{

	System.out.println("Entra no loginFacebook");
	for (Users obj : b.users)
	{
	System.out.println("inside for"+IDcode);
	System.out.println("o que la está"+obj.getFID()+" | "+"o que procurei "+IDcode);
	if (obj.getFID().equals(IDcode))
	{
		System.out.println("Ja ca estava!!!!!!!!");
		Login(obj.getNome(), obj.getPassword());
	if (obj.admin) return 1;
		else return 2;
...
@Override
public boolean LoginFacebookInside(String nome, String ID)
{		System.out.println("ENTROU NO RMI");
	for (Users obj : b.users)
	{
	System.out.println("inside for");
	System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!o que esta lá"+obj.nome+"o que procurei"+nome);
	if (obj.nome.equals(nome))
	{
		
		obj.FID = ID;
		System.out.println("AQUI"+ obj.FID+" "+obj.nome);
	return true;
...
@Override
public boolean LoginFacebookAdmin(String nome, String ID)
{
	for (Users obj : b.users)
	{
	System.out.println("inside for");
